...
def programar_apagado(entry):
    ...
    try:
        # Obtener la hora ingresada y convertirla a minutos restantes
        hora_apagado = entry.get()  # Por ejemplo: "18:00"
        hora_objetivo = time.strptime(hora_apagado, "%H:%M")
        minutos_objetivo = hora_objetivo.tm_hour * 60 + hora_objetivo.tm_min

        # Calcular la diferencia en minutos desde el momento actual
        ahora = time.localtime()
        minutos_ahora = ahora.tm_hour * 60 + ahora.tm_min
        minutos_restantes = minutos_objetivo - minutos_ahora

        if minutos_restantes <= 0:
            minutos_restantes += 1440  # Añadir un día (1440 minutos) si ya pasó la hora

        # Ejecutar el comando de apagado con el tiempo restante en minutos
        os.system(f"shutdown +{minutos_restantes}")
        messagebox.showinfo("Apagado Programado", f"El equipo se apagará en {minutos_restantes // 60} horas y {minutos_restantes % 60} minutos.")
    except ValueError:
...
def interfaz_principal():
    ...
    copiar_button = tk.Button(ventana_principal, text="Crear copia de seguridad", width=20, height=3 ,command=lambda: subprocess.Popen(['bash', "crear_copia.sh"]))#Igualamos a lambda para que command nos permita pasar argumentos a la funcion
